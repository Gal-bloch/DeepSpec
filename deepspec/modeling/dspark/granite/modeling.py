# DSpark draft model for IBM Granite dense targets (model_type "granite",
# GraniteForCausalLM). Structure mirrors deepspec/modeling/dspark/qwen3/modeling.py;
# the Granite-specific deltas are:
#   * no q_norm / k_norm (Granite attention has none),
#   * attention scaling uses config.attention_multiplier (not head_dim**-0.5),
#   * embeddings scaled by config.embedding_multiplier,
#   * each residual add scaled by config.residual_multiplier,
#   * final logits divided by config.logits_scaling.
# Granite scaling semantics adapted from transformers' modeling_granite.py
# (Apache-2.0, IBM & HuggingFace 2024).
...
class GraniteDSparkAttention(nn.Module):
    def __init__(self, config, layer_idx: int):
        ...
        # Granite: attention scaling is a configured multiplier, not head_dim**-0.5.
        ...
        # Granite attention has no q_norm / k_norm.
...
class GraniteDSparkDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.residual_multiplier = config.residual_multiplier
        self.self_attn = GraniteDSparkAttention(config=config, layer_idx=layer_idx)
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Granite scales each residual branch by residual_multiplier.
...
class GraniteDSparkModel(GranitePreTrainedModel):
    _no_split_modules = ["GraniteDSparkDecoderLayer"]
    ...
    def __init__(self, config) -> None:
        ...
        self.layers = nn.ModuleList(
            [
                GraniteDSparkDecoderLayer(config, layer_idx)
        ...
        self.hidden_norm = GraniteRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
    ...
    def compute_logits(self, hidden_states: torch.Tensor) -> torch.Tensor:
        # Granite divides logits by logits_scaling.
        return self.lm_head(hidden_states) / self.logits_scaling
    ...
    def forward(
        ...
        # Granite scales input embeddings by embedding_multiplier.
        noise_embedding = noise_embedding * self.embedding_multiplier
...
__all__ = [
    "GraniteDSparkModel",
    "GraniteDSparkAttention",
    "GraniteDSparkDecoderLayer",
]
